...
.metric-card h2 { margin: 0; font-size: 2rem; color: #1a73e8; }
...
COLOR_MAP = {
    "Insufficient_Weight":  "#4fc3f7",
    "Normal_Weight":        "#66bb6a",
    "Overweight_Level_I":   "#ffee58",
    "Overweight_Level_II":  "#ffa726",
    "Obesity_Type_I":       "#ef5350",
    "Obesity_Type_II":      "#c62828",
    "Obesity_Type_III":     "#6a1b1a",
}

BG_MAP = {
    "Insufficient_Weight":  ("background:#e3f2fd;color:#0d47a1", "💧"),
    "Normal_Weight":        ("background:#e8f5e9;color:#1b5e20", "✅"),
    "Overweight_Level_I":   ("background:#fffde7;color:#f57f17", "⚠️"),
    "Overweight_Level_II":  ("background:#fff3e0;color:#e65100", "⚠️"),
    "Obesity_Type_I":       ("background:#fce4ec;color:#880e4f", "🔴"),
    "Obesity_Type_II":      ("background:#f3e5f5;color:#6a1b9a", "🔴"),
    "Obesity_Type_III":     ("background:#fbe9e7;color:#bf360c", "🚨"),
}

RECOMMENDATIONS = {
    "Insufficient_Weight": [
        "Consultar nutricionista para plano de ganho de peso saudável.",
        "Aumentar ingesta calórica com alimentos nutritivos.",
...
@st.cache_resource
def load_model():
    """Carrega modelo treinado ou treina na hora."""
    model_dir = "model"
    ...
    if all(os.path.exists(os.path.join(model_dir, f)) for f in required):
        ...
        # Treina LightGBM na hora se não houver modelo salvo
        return _train_fallback_model()


def _train_fallback_model():
    """Treina um modelo LightGBM rapidamente como fallback."""
    ...
    for col in ["FCVC", "NCP", "CH2O", "FAF", "TUE"]:
        df_work[col] = df_work[col].round().astype(int)
    df_work["BMI"]          = df_work["Weight"] / (df_work["Height"] ** 2)
    df_work["risk_score"]   = (
        (df_work["FAVC"] == "yes").astype(int) * 2 +
        (df_work["family_history"] == "yes").astype(int) * 2 +
        (df_work["SMOKE"] == "yes").astype(int) +
        (df_work["SCC"] == "no").astype(int) +
        (df_work["CAEC"] == "Always").astype(int) * 2 +
        (df_work["CAEC"] == "Frequently").astype(int) +
        (df_work["CALC"] == "Always").astype(int) * 2 +
        (df_work["CALC"] == "Frequently").astype(int)
    )
    df_work["healthy_score"] = (
        (df_work["FAF"] >= 2).astype(int) * 2 +
        (df_work["FCVC"] >= 2).astype(int) +
        (df_work["CH2O"] >= 2).astype(int) +
        (df_work["SCC"] == "yes").astype(int) +
        df_work["MTRANS"].isin(["Walking", "Bike"]).astype(int)
    )
    ...
    df_work["CAEC"] = df_work["CAEC"].map(caec_map)
    df_work["CALC"] = df_work["CALC"].map(calc_map)
    df_work = pd.get_dummies(df_work, columns=["MTRANS"], drop_first=False)
    ...
    X = df_work.drop(columns=["Obesity"])
    y = le.transform(df_work["Obesity"])
...
def predict_obesity(inputs: dict, model, scaler, le, meta) -> tuple:
    """Retorna (classe_predita, probabilidades_dict)."""
    ...
    if hasattr(model, "predict_proba"):
        ...
        proba = {le.inverse_transform([i])[0]: float(p)
...
# Pre-processamento do dataset para o dashboard
df_dash = df_raw.copy()
...
with st.sidebar:
    ...
    st.markdown(f"**Dataset:** {len(df_raw):,} registros")
    st.markdown("---")
...
if page == "🔮 Sistema Preditivo":
    ...
    with st.form("prediction_form"):
        ...
        c5.metric("IMC Calculado", f"{bmi_calc:.1f} kg/m²",
                  delta="Normal" if 18.5 <= bmi_calc < 25 else
                        ("Abaixo" if bmi_calc < 18.5 else "Acima"))
        ...
        caec = c9.selectbox("Come entre as refeições?",
                             ...
                             format_func=lambda x: {"no": "Não", "Sometimes": "Às vezes",
                                                    ...
                                                    "Always": "Sempre"}[x])
        ch2o = c10.slider("Consumo diário de água (1=<1L / 2=1–2L / 3=>2L)", 1, 3, 2)

        st.markdown("---")
        st.markdown('<div class="section-title">🏃 Estilo de Vida</div>',
                    unsafe_allow_html=True)
        c11, c12, c13 = st.columns(3)
        faf   = c11.slider("Frequência de atividade física/semana (0=Nenhuma / 3=Diária)", 0, 3, 1)
        tue   = c12.slider("Tempo em dispositivos eletrônicos (0=<2h / 1=3–5h / 2=>5h)", 0, 2, 1)
        ...
        family_history = st.selectbox(
            "Familiar com histórico de excesso de peso?", ["no", "yes"],
            format_func=lambda x: "Não" if x == "no" else "Sim"
        )
    ...
    if submitted:
        ...
        style, icon = BG_MAP[pred_class]
        st.markdown(f"""
        <div class="pred-box" style="{style}">
            <h1>{icon} {LABEL_PT[pred_class]}</h1>
        ...
        if proba:
            ...
            fig_prob = px.bar(
                ...
                color="Probabilidade",
                color_continuous_scale=["#66bb6a", "#ffee58", "#ef5350"],
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    "steps": [
                        ...
                        {"range": [25, 30],    "color": "#fffde7"},
                        {"range": [30, 35],    "color": "#fff3e0"},
                        {"range": [35, 40],    "color": "#fce4ec"},
                        {"range": [40, 50],    "color": "#fbe9e7"},
                    ],
                    "threshold": {
        ...
        with st.expander("📋 Resumo dos Dados Informados"):
            resumo = pd.DataFrame({
                "Campo": [
                    ...
                    "Histórico familiar", "Alimentos calóricos", "Vegetais",
                    "Refeições/dia", "Come entre refeições", "Fuma",
                    "Água/dia", "Monitora calorias", "Atividade física",
                ...
                "Valor": [
                    ...
                    f"{faf}/3", f"{tue}/2",
                    {"no": "Não bebe", "Sometimes": "Às vezes",
...
# ══════════════════════════════════════════════════════════
    ...
    # ── KPIs ──────────────────────────────────────────
    total     = len(df_dash)
    obesos    = df_dash["Obesity"].isin(
        ["Obesity_Type_I", "Obesity_Type_II", "Obesity_Type_III"]
    ).sum()
    sobrepeso = df_dash["Obesity"].isin(
        ["Overweight_Level_I", "Overweight_Level_II"]
    ).sum()
    ...
    k1.markdown(f"""<div class="metric-card"><h2>{total:,}</h2>
                    <p>Total de Pacientes</p></div>""", unsafe_allow_html=True)
    k2.markdown(f"""<div class="metric-card" style="color:#ef5350">
                    <h2 style="color:#ef5350">{obesos:,}</h2>
                    <p>Com Obesidade ({obesos/total*100:.0f}%)</p></div>""",
                unsafe_allow_html=True)
    k3.markdown(f"""<div class="metric-card"><h2 style="color:#ffa726">{sobrepeso:,}</h2>
                    <p>Sobrepeso ({sobrepeso/total*100:.0f}%)</p></div>""",
                unsafe_allow_html=True)
    k4.markdown(f"""<div class="metric-card"><h2 style="color:#66bb6a">{normal:,}</h2>
                    <p>Peso Normal ({normal/total*100:.0f}%)</p></div>""",
                unsafe_allow_html=True)
    k5.markdown(f"""<div class="metric-card"><h2>{imc_medio:.1f}</h2>
                    <p>IMC Médio (kg/m²)</p></div>""", unsafe_allow_html=True)
    ...
    st.markdown(f"*{len(df_f):,} registros após filtros aplicados*")
    st.markdown("---")

    # ── ROW 1 ──────────────────────────────────────────
    st.markdown('<div class="section-title">Distribuição & Perfil Demográfico</div>',
                unsafe_allow_html=True)
    ...
    with r1c1:
        ...
        dist["Label"] = dist["Obesity"].map(LABEL_PT)
        fig1 = px.bar(
    ...
    with r1c2:
        gender_dist = (
            df_f.groupby(["Obesity", "Gender"])
            .size().reset_index(name="Count")
        )
        gender_dist["Label"] = gender_dist["Obesity"].map(LABEL_PT)
        gender_dist["Gender_PT"] = gender_dist["Gender"].map(
            {"Female": "Feminino", "Male": "Masculino"}
        )
        fig2 = px.bar(
            gender_dist,
            x="Label", y="Count", color="Gender_PT",
            barmode="group",
            title="Distribuição por Gênero e Nível de Obesidade",
            labels={"Label": "", "Count": "Pacientes", "Gender_PT": "Gênero"},
            color_discrete_map={"Feminino": "#f48fb1", "Masculino": "#90caf9"},
        )
    ...
    # ── ROW 2 ──────────────────────────────────────────
    st.markdown("---")
    st.markdown('<div class="section-title">IMC, Peso e Idade por Nível de Obesidade</div>',
                unsafe_allow_html=True)
    r2c1, r2c2 = st.columns(2)

    with r2c1:
        bmi_box = df_f.copy()
        bmi_box["Label"] = bmi_box["Obesity"].map(LABEL_PT)
        fig3 = px.box(
            bmi_box, x="Label", y="BMI",
            color="Obesity",
            color_discrete_map=COLOR_MAP,
            title="Distribuição do IMC por Nível de Obesidade",
            labels={"Label": "", "BMI": "IMC (kg/m²)"},
            category_orders={"Label": [LABEL_PT[o] for o in ORDER]},
        )
        fig3.add_hline(y=18.5, line_dash="dash", line_color="blue",
                       annotation_text="18.5 (Abaixo do Peso)")
        fig3.add_hline(y=25, line_dash="dash", line_color="green",
                       annotation_text="25 (Normal)")
        fig3.add_hline(y=30, line_dash="dash", line_color="orange",
                       annotation_text="30 (Sobrepeso)")
    ...
    with r2c2:
        ...
        scatter_df["Label"] = scatter_df["Obesity"].map(LABEL_PT)
        fig4 = px.scatter(
            scatter_df.sample(min(600, len(scatter_df)), random_state=42),
            x="Age", y="BMI",
            color="Obesity",
            color_discrete_map=COLOR_MAP,
            opacity=0.65,
    ...
    with r3c1:
        ...
        fam_pct.columns = ["Obesity", "Pct"]
        fam_pct["Label"] = fam_pct["Obesity"].map(LABEL_PT)
        fig5 = px.bar(
            fam_pct, x="Label", y="Pct",
            color="Obesity", color_discrete_map=COLOR_MAP,
            title="% com Histórico Familiar",
            labels={"Label": "", "Pct": "%"},
            text=fam_pct["Pct"].round(1).astype(str) + "%",
        )
    ...
    with r3c2:
        faf_mean = (
            df_f.groupby("Obesity")["FAF"]
            .mean().reindex(ORDER).reset_index()
        )
        faf_mean["Label"] = faf_mean["Obesity"].map(LABEL_PT)
        fig6 = px.bar(
            faf_mean, x="Label", y="FAF",
            color="Obesity", color_discrete_map=COLOR_MAP,
            title="Atividade Física Média (FAF)",
            labels={"Label": "", "FAF": "FAF médio (0–3)"},
            text=faf_mean["FAF"].round(2),
        )
    ...
    with r3c3:
        ...
        favc_pct.columns = ["Obesity", "Pct"]
        favc_pct["Label"] = favc_pct["Obesity"].map(LABEL_PT)
        fig7 = px.bar(
            favc_pct, x="Label", y="Pct",
            color="Obesity", color_discrete_map=COLOR_MAP,
            title="% que Consome Alimentos Calóricos",
            labels={"Label": "", "Pct": "%"},
            text=favc_pct["Pct"].round(1).astype(str) + "%",
        )
    ...
    # ── ROW 4 ──────────────────────────────────────────
    st.markdown("---")
    st.markdown('<div class="section-title">Transporte, Água e Álcool</div>',
                unsafe_allow_html=True)
    ...
    with r4c1:
        mtrans_df = (
            df_f.groupby(["MTRANS", "Obesity"])
            .size().reset_index(name="Count")
        )
        mtrans_df["MTRANS_PT"] = mtrans_df["MTRANS"].map({
        ...
        mtrans_df["Label"] = mtrans_df["Obesity"].map(LABEL_PT)
        fig8 = px.bar(
            mtrans_df, x="MTRANS_PT", y="Count",
            color="Obesity", color_discrete_map=COLOR_MAP,
            title="Meio de Transporte por Nível de Obesidade",
            labels={"MTRANS_PT": "Transporte", "Count": "Pacientes"},
            barmode="stack",
        )
        fig8.update_layout(legend_title="Nível", margin=dict(t=40))
    ...
    insights = [
        ("🧬 Fator Genético", f"{df_f[df_f['Obesity'].isin(['Obesity_Type_I','Obesity_Type_II','Obesity_Type_III'])]['family_history'].value_counts(normalize=True).get('yes',0)*100:.0f}% dos pacientes obesos possuem histórico familiar de excesso de peso — o fator hereditário é determinante no risco."),
        ("🏃 Sedentarismo", f"Pacientes com Obesidade Tipo III praticam em média {df_f[df_f['Obesity']=='Obesity_Type_III']['FAF'].mean():.1f}/3 de atividade física semanal, contra {df_f[df_f['Obesity']=='Normal_Weight']['FAF'].mean():.1f}/3 no grupo de Peso Normal."),
        ("🍔 Alimentação Calórica", f"{df_f[df_f['Obesity'].isin(['Obesity_Type_I','Obesity_Type_II','Obesity_Type_III'])]['FAVC'].value_counts(normalize=True).get('yes',0)*100:.0f}% dos pacientes obesos consomem alimentos altamente calóricos frequentemente."),
        ("📊 Monitoramento Calórico", f"Apenas {df_f[df_f['SCC']=='yes'].shape[0]/len(df_f)*100:.1f}% dos pacientes monitoram sua ingestão calórica — intervenção educativa de alto impacto potencial."),
        ("🚗 Transporte Passivo", f"{df_f[df_f['MTRANS'].isin(['Automobile','Motorbike'])].shape[0]/len(df_f)*100:.0f}% dos pacientes usam transporte motorizado, reduzindo a atividade física diária."),
        ("💧 Hidratação", f"O consumo adequado de água (≥2L/dia) é maior no grupo de Peso Normal ({df_f[df_f['Obesity']=='Normal_Weight']['CH2O'].mean():.1f}/3) versus Obesidade Tipo III ({df_f[df_f['Obesity']=='Obesity_Type_III']['CH2O'].mean():.1f}/3)."),
    ]
